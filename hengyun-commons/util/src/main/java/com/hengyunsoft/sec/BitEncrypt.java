...
//   static byte firstJM = 84;
//   static byte secondJM = 43;

    private static void replace127(byte[] result) {
    ...
    private static void unReplace127(byte[] result) {
        
        for (int i = 0,len = result.length; i < len; i++) {
            if(result[i] == replace127)
                result[i] = byteMax;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 1000; i++) {
        	...
        	System.out.println(unencoding(ss));
        	System.out.println();
        }
    }
