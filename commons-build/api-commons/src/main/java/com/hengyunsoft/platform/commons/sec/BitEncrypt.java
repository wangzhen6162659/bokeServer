...
//    private final static byte secondTemplate = 0X6C;
    public static String encoding(String str){
        ...
        if(str == null || str.length() == 0)
            return str;
        byte[] encoding = encoding(str.getBytes());
...
//        replace127(result);
    ...
    private static void replaceSpecialByte(byte[] result) {
        ...
        for (int i = 0,len = result.length; i < len; i++) {
            switch (result[i]) {
                case byteMax: result[i] = replace127 ;break;
                case special92 : result[i] = replace92 ;break;
            }
        }
    }
    ...
    private static void unReplaceSpecialByte(byte[] result) {
        ...
        for (int i = 0,len = result.length; i < len; i++) {
            switch (result[i]) {
                case replace127: result[i] = byteMax ;break;
                case replace92 : result[i] = special92 ;break;
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        String txt = "ptZgbPz``u^CtgnYfJPjfbLGjcXoNOzIHZbQPWjvjI@{NdfyTJP{znnE`}JJZz`+RwLA|cNwzAh_huZNXWdT+qtJhZ+KxqNFRkLiVsHAlEVdVaFcxEpgxhBvVWTSxilWnaLvxGnvjUPppXtm|ZXDnlbVZqxNf^nCNAvbnnpSTwfxntverUXzDiD|Jgj^Thd}LH@KLkNchJdCPuVM~zPA|b|MTdzCZdhEFLBB+q^nZiTCF+dtJeZtrzZApg|bZIxyt}+[pFPVjblcVHxcXr`Kpx+HZyjEf@jqTaXoxyvoj`zGL}ZqhOVHdMrT+iVwfTpV@GBAjAzypvzfLkbrPZFDLlB}~PztPhPsdA^MrRNepozAT]`ZpjLWXarVBcRqrqvwpyvObbtp@IrElldrndJvj}h]lSdux[nxdZzzhwRFj]~fJIFe~Q|WHV^V|pZGPH|yT[hYPPlaT[n]PGNwtbN_zAJtlVbvpsvelvVlt^Hyj!xwvr^WFdfYbwN_FSjktL|`tlpYBqfvXPjNDZfPnSVW|DtWb!Hw^E`vj{vdrrD!ZEPpDo|gfmPs^UL{VF|rfetaZ_T{By^kecoxqa]gQp_DEwKUShht^_JU_k[_!ui!wwj^!cuk!^o[UnzU!_lS!wk_dfomP_![]nR_o!hk_Wn!eZwIsWx_]J~Fn__!U}hw}[U{{ZjF}oVo~genuuVcyok_Hne@o";
        for (int i = 0; i < 1000; i++) {
            String ss = encoding(txt);
            System.out.println(ss);
            System.out.println(unencoding(ss).equals(txt));
            System.out.println();
        }
    }
